...
function renderReceiptHtml(transaction){
  ...
  return `<div class="receipt-content"><header class="receipt-header"><div class="store-name">SUMBER BERKAH SIDOGURO</div><div class="store-address">ROWO JOMBOR / TRUCUK, KLATEN</div><div class="store-phone">081327375989 / 085727435699</div></header><div class="receipt-divider"></div><section class="receipt-meta"><div class="receipt-row"><span class="label">Tanggal</span><span class="value">${esc(parts.date)}</span></div><div class="receipt-row"><span class="label">Jam</span><span class="value">${esc(parts.time)}</span></div><div class="receipt-row"><span class="label">No. Nota</span><span class="value">${esc(transaction.invoice)}</span></div><div class="receipt-row"><span class="label">Kasir</span><span class="value">${esc(transaction.cashier)}</span></div><div class="receipt-row"><span class="label">Pengantar</span><span class="value">${esc(transaction.courier||"")}</span></div><div class="receipt-row customer-row"><span class="label">Pelanggan</span><span class="value">${esc(transaction.customer||"UMUM")}</span></div></section><div class="receipt-divider"></div><section class="receipt-items">${items}</section><div class="receipt-divider"></div><section class="receipt-summary"><div class="receipt-row"><span class="label">Subtotal</span><span class="value">${rupiah(transaction.subtotal)}</span></div><div class="receipt-row"><span class="label">Diskon</span><span class="value">${rupiah(transaction.discountAmount)}</span></div><div class="receipt-row"><span class="label">Pajak</span><span class="value">${rupiah(transaction.taxAmount)}</span></div><div class="receipt-row total-row"><span class="label">Total</span><span class="value">${rupiah(transaction.total)}</span></div>${receiptPaymentHtml(transaction)}</section><footer class="receipt-footer"><div>Terimakasih atas pembelian Anda</div><div class="footer-message">simpan nota ini sebagai bukti transaksi</div><div>TF MANDIRI : 1380023004349 Farisa</div></footer></div>`;
}

function receiptCss(root){return `${root}{box-sizing:border-box;width:58mm;max-width:58mm;margin:0 auto;padding:0;background:#fff;color:#000;overflow:visible}${root} *{box-sizing:border-box;max-width:100%}${root} .receipt-content{width:54mm;max-width:54mm;margin:0 auto;padding:1mm .5mm 1.5mm;font-family:Arial,Helvetica,sans-serif;font-size:10.5px;font-weight:400;line-height:1.15}${root} .receipt-header{text-align:center;line-height:1.1}${root} .store-name{width:100%;max-width:100%;text-align:center;font-size:14px;font-weight:700;line-height:1.05;letter-spacing:-.1px;white-space:nowrap}${root} .store-address,${root} .store-phone{font-size:10px;font-weight:400;line-height:1.1;white-space:nowrap}${root} .store-address{margin-top:.6mm}${root} .store-phone{margin-top:.3mm}${root} .receipt-divider{margin:.8mm 0;border-top:1px dashed #000}${root} .receipt-row{display:grid;grid-template-columns:18mm minmax(0,1fr);column-gap:1.5mm;align-items:start;min-height:0;line-height:1.15}${root} .receipt-row .label,${root} .receipt-row .value{min-width:0;font-weight:400}${root} .receipt-row .value{text-align:right;overflow-wrap:anywhere;white-space:normal}${root} .customer-row{margin:.6mm 0}${root} .customer-row .label,${root} .customer-row .value{font-weight:700}${root} .customer-row .value{overflow-wrap:break-word;word-break:normal;white-space:normal}${root} .receipt-item{margin:.7mm 0;line-height:1.15;break-inside:avoid;page-break-inside:avoid}${root} .item-name{margin:0;font-weight:400;overflow-wrap:anywhere}${root} .item-detail{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:1.5mm;margin-top:.15mm;font-weight:400}${root} .item-detail span{min-width:0;overflow-wrap:anywhere}${root} .item-detail span:last-child{text-align:right;white-space:nowrap}${root} .receipt-summary{line-height:1.15;break-inside:avoid;page-break-inside:avoid}${root} .total-row{margin:.5mm 0;padding:.55mm 0;border-top:1px dashed #000;border-bottom:1px dashed #000;font-size:12px;line-height:1.1}${root} .total-row .label,${root} .total-row .value{font-weight:700}${root} .receipt-footer{margin-top:.8mm;text-align:center;font-size:9.5px;font-weight:400;line-height:1.15;overflow-wrap:anywhere;break-inside:avoid;page-break-inside:avoid}${root} .receipt-footer>div{margin-top:.35mm}${root} .receipt-footer>div:first-child{margin-top:0}${root} .footer-message{white-space:nowrap}`}
